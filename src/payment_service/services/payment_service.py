...
from payment_service.models.payment import (
    ...
    RefundRequest, RefundResponse, PaymentStatus, RefundStatus
)
...
class PaymentService:
    ...
    async def process_payment(self, payment_request: PaymentRequest, correlation_id: str) -> PaymentResponse:
        ...
        with create_span("payment.process", resource="process_payment"):
            ...
            try:
                ...
                if payment_request.card_data:
                    encrypted_card_data = self.encryption_service.encrypt_card_data(payment_request.card_data)
                ...
                response = PaymentResponse(
                    ...
                    card_last_four=card_last_four,
                    authorization_id=authorization_result.get("authorization_id"),
                    capture_id=capture_result.get("capture_id") if final_status == PaymentStatus.CAPTURED else None,
                ...
                await self._update_transaction_status(transaction_id, PaymentStatus.FAILED, correlation_id)
                
                increment_counter("payment.failed", tags={"error": type(e).__name__})
                raise
    
    async def get_payment_status(self, transaction_id: str, correlation_id: str) -> PaymentStatusResponse:
    ...
    async def process_refund(
        ...
        with create_span("payment.refund", resource="process_refund"):
            ...
            try:
                ...
                final_status = RefundStatus.COMPLETED if refund_result.get("status") == "refunded" else RefundStatus.FAILED
                ...
                response = RefundResponse(
                    ...
                    updated_at=datetime.utcnow(),
                    processed_at=datetime.utcnow() if final_status == RefundStatus.COMPLETED else None,
                )
                ...
                await self._update_refund_status(refund_id, RefundStatus.FAILED, None, correlation_id)
    ...
    async def _update_transaction_authorization(
        ...
        status = PaymentStatus.AUTHORIZED if authorization_result.get("status") == "approved" else PaymentStatus.FAILED
        
        await database_manager.execute_query(
            query,
            (authorization_result.get("authorization_id"), status.value, transaction_id)
        )
    ...
    async def _update_transaction_capture(
        ...
        await database_manager.execute_query(
            query,
            (capture_result.get("capture_id"), transaction_id)
        )
    ...
    async def _update_refund_status(
        ...
        await database_manager.execute_query(
            query, 
            (status.value, external_refund_id, status.value, refund_id)
        )
